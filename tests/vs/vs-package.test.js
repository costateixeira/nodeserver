...
describe('PackageValueSetProvider', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      provider = new PackageValueSetProvider(fullPackagePath);
  ...
  describe('Database Debug', () => {
    test('should be able to insert systems directly', async () => {
      // Test direct database access to isolate the issue
      const sqlite3 = require('sqlite3').verbose();
      const dbPath = path.join(packageCacheDir, packagePath, '.valuesets.db');
      ...
      await new Promise((resolve, reject) => {
        const db = new sqlite3.Database(dbPath, (err) => {
          ...
          // Test simple insert
          const testUrl = 'http://test.example.com/ValueSet/test';
          const testSystem = 'http://test.example.com/CodeSystem/test';

          console.log('Testing direct database insert...');

          db.run('INSERT INTO valueset_systems (valueset_url, system) VALUES (?, ?)',
            [testUrl, testSystem],
            function(err) {
              if (err) {
              ...
              console.log(`Direct insert succeeded, changes: ${this.changes}`);

              // Verify it's there
              db.all('SELECT * FROM valueset_systems WHERE valueset_url = ?', [testUrl], (err, rows) => {
                if (err) {
                  console.error('Verification query failed:', err);
                  db.close();
                  reject(err);
                } else {
                  console.log(`Verification found ${rows.length} rows:`, rows);
                  expect(rows.length).toBe(1);
                  expect(rows[0].system).toBe(testSystem);
    ...
    test('should show actual table schema', async () => {
      const sqlite3 = require('sqlite3').verbose();
      const dbPath = path.join(packageCacheDir, packagePath, '.valuesets.db');
      ...
      await new Promise((resolve) => {
        const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READONLY, (err) => {
          ...
          // Check table schema
          db.all('SELECT sql FROM sqlite_master WHERE name="valueset_systems"', [], (err, rows) => {
            if (err) {
              console.error('Error querying schema:', err);
            } else {
              console.log('valueset_systems table schema:');
              rows.forEach(row => console.log(row.sql));
            }

            // Check if table has any data at all
            db.all('SELECT COUNT(*) as count FROM valueset_systems', [], (err, countRows) => {
              if (err) {
                console.error('Error counting rows:', err);
              } else {
                console.log(`Total rows in valueset_systems: ${countRows[0].count}`);
              }
